...
namespace TGMWalletCore.Helper.Http
{
    public class Client
    {
        ...
        {
            _apiRestSection = apiRestSection.GetSection(Constant.ApiGateway);
            _logger = logger;
        }
        ...
        public async Task<TaskResult<byte[]>> PostAsync<T>(T payload, RestApiMethod apiMethod) where T : class
        {
            ...
            using (var cts = new CancellationTokenSource())
            {
                try
                {
                    ...
                    var path = _apiRestSection.GetSection(Constant.Routing).GetValue<string>(apiMethod.ToString());
            ...
            using (var cts = new CancellationTokenSource())
            {
                try
                {
                    ...
                    path = apiMethod.ToString().Equals(Constant.GetCoin)
                        ? string.Format(_apiRestSection.GetSection(Constant.Routing).GetValue<string>(apiMethod.ToString()), address, args[0])
                        : string.Format(_apiRestSection.GetSection(Constant.Routing).GetValue<string>(apiMethod.ToString()), address);
        ...
        public async Task<IEnumerable<T>> GetRangeAsync<T>(string address, int skip, int take, RestApiMethod apiMethod) where T : class
        {
            ...
            using (var cts = new CancellationTokenSource())
            {
                ...
                {
                    var baseAddress = GetBaseAddress();
                    var path = string.Format(_apiRestSection.GetSection(Constant.Routing).GetValue<string>(apiMethod.ToString()), address, skip, take);
        ...
        {
            return new Uri(_apiRestSection.GetValue<string>(Constant.Endpoint));
        }
